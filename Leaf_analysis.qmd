...
```{R}
# Fit a linear regression model
model <- lm(ml ~ x, data = df_3)

# Create a scatter plot
plot(df_3$x, df_3$ml, main = "Mass per Leaf vs. Distance from Main Colony Entrance", xlab = "Distance (m)", ylab = "Mass per Leaf", col = "purple", pch = 16)
...
text(min(df_3$x), max(df_3$ml), equation, pos = 4, col = "purple")
...
# Add R-squared value to the plot
r_squared_text <- paste("R-squared =", round(r_squared, 3))
text(min(df_3$x), max(df_3$ml) - 0.1 * diff(range(df_3$ml)), r_squared_text, pos = 4, col = "black")
```

### Plot Mass per Area vs. Distance
```{R}
# Fit a linear regression model
model <- lm(ma ~ x, data = df_3)

# Create a scatter plot
plot(df_3$x, df_3$ma, main = "Mass per Area vs. Distance from Main Colony Entrance", xlab = "Distance (m)", ylab = "Mass per Area", col = "orange", pch = 16)
...
# Add the equation of the line
equation <- paste("y =", round(coefficients[1], 2), "+", round(coefficients[2], 2), "* x")
text(min(df_3$x), max(df_3$ma), equation, pos = 4, col = "orange")
...
# Add R-squared value to the plot
r_squared_text <- paste("R-squared =", round(r_squared, 3))
text(min(df_3$x), max(df_3$ma) - 0.1 * diff(range(df_3$ma)), r_squared_text, pos = 4, col = "black")
```
